...
EXPORT ChildAutismRec := RECORD
    STRING50 State;
    REAL4 AutismRate;
END;

EXPORT ChildAutismDS := DATASET('~hmk::in::autism_prevalance.csv', ChildAutismRec, CSV(HEADING(1)));

EXPORT CrimeRateRec := RECORD
    STRING50 State;
    REAL4 CrimeRate;
END;

EXPORT CrimeRateDS := DATASET('~hmk::in::crimerate_by_state.csv', CrimeRateRec, CSV(HEADING(1)));
...
EXPORT AirportDS := DATASET('~hmk::in::us_airport.csv', AirportRec,CSV(HEADING(1)));


EXPORT TruckStopRec := RECORD
    STRING50 RestStopName;
    STRING50 Highway_Route;
    REAL4 Mile_Post;
    STRING50 City;
    STRING50 County;
    STRING5 State;
...
EXPORT TruckStopDS := DATASET('~hmk::in::truck_stop_parking', TruckStopRec, CSV(HEADING(1)));
END;